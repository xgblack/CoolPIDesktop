...
async fn resume_exact_session_and_history() {
    ...
    let result=tokio::spawn(async move {
        ...
        let state=worker.query("first","get_state",json!({})).await.unwrap();
        ...
        worker.request("first","prompt",json!({"message":format!("Remember this marker: {marker}. Do not use tools. Reply only SAVED.")})).await.unwrap();
        ...
        let page=worker.query("resumed","get_messages_page",json!({"limit":2})).await.unwrap();
        assert!(page.to_string().contains(&marker));
        worker.request("resumed","prompt",json!({"message":"What marker did I ask you to remember? Reply with only that marker. Do not use tools."})).await.unwrap();
