...
async fn real_trajectory_stream_tools_persistence_and_cancel() {
  ...
  let state=worker.query("trajectory",host_core::RpcQuery::GetState,json!({})).await.unwrap();
  let session=state["sessionId"].as_str().unwrap().to_owned();let file=std::path::PathBuf::from(state["sessionFile"].as_str().unwrap());
  worker.request("trajectory",host_core::RpcRequest::Prompt,json!({"message":"Use your read tool to read trajectory-input.txt in the current directory. Do not edit any files. Reply with the file contents only."})).await.unwrap();
  ...
  worker.request("trajectory",host_core::RpcRequest::Prompt,json!({"message":"Do not use tools. Write a detailed 2000 word explanation of database indexing."})).await.unwrap();
  ...
  worker.request("trajectory",host_core::RpcRequest::Abort,json!({})).await.unwrap();
