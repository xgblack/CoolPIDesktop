...
async fn database_reopen_preserves_task_and_session() {
  ...
  w.request(&other.id,"prompt",json!({"message":format!("Use tools to read marker.txt in your current working directory and {}. Return the exact contents of both files. Do not change files.",root.join("additional/marker.txt").display())})).await.unwrap();
  ...
  w.request(&task.id,"prompt",json!({"message":format!("Remember {marker}. Do not use tools. Reply SAVED.")})).await.unwrap();idle(&w,&task.id).await;
  ...
  let page=w.runtime.query(&task.id,"get_messages_page",json!({"limit":1})).await.unwrap();
  assert_eq!(page["messages"].as_array().unwrap().len(),1);
  let cursor=page["nextCursor"].as_str().unwrap();
  let next=w.runtime.query(&task.id,"get_messages_page",json!({"limit":1,"cursor":cursor})).await.unwrap();
  ...
  let outcome=async {
   ...
   reopened.request(&task.id,"prompt",json!({"message":"What marker did I give you? Reply only the marker, no tools."})).await.unwrap();idle(&reopened,&task.id).await;
   assert!(reopened.runtime.snapshot(&task.id).await.unwrap().text.contains(&marker));
   assert_eq!(reopened.runtime.query(&task.id,"get_messages_page",json!({"cursor":cursor,"limit":1})).await.unwrap_err().code,"stale_cursor");
