...
export function App(){
 ...
 useEffect(()=>{if(w.projectId&&w.models.length&&!w.projectModels[w.projectId])w.setProjectModels(old=>({...old,[w.projectId]:w.models[0].provider+'/'+w.models[0].id}));},[w.projectId,w.models]);
 const {task,run}=w;
 ...
 const content=<main ref={main} tabIndex={-1} className="conversation">
  ...
  {task?<><MessageList tools={run?.tools} taskKey={key} messages={w.messages} streamingText={running||w.historyBusy?run?.text??'':''} running={running} hasMore={!!w.cursor} loading={w.historyBusy} onMore={()=>{if(!running)void w.history(true);}} onRefresh={()=>{if(!running)void w.history();}} onError={e=>w.setError(hostError(e))}/>
   ...
   <Composer key={`composer:${task.id}`} taskId={task.id} roots={task.roots} references={w.references[task.id]??[]} onReferences={refs=>w.setReferences(old=>({...old,[task.id]:refs}))} models={ready||running?capabilities?.models??[]:w.models} modelValue={ready||running?capabilities?.state?.model?capabilities.state.model.provider+'/'+capabilities.state.model.id:undefined:task.model??undefined} modelsLoading={!ready&&!running&&w.modelsLoading} modelsError={!ready&&!running?w.modelsError?.message:undefined} onRefreshModels={ready||running?undefined:w.refreshModels} onModel={(p,id)=>void w.act(()=>records.model(task.id,p,id))} draft={w.drafts[task.id]??''} onFiles={()=>setDetails(true)} onContinue={()=>void w.act(()=>records.resume(task.id))} onSettings={()=>setSettings(true)} modelLabel={ready||running?capabilities?.state?.model?.id:undefined} attachmentIds={w.attachments[task.id]??[]} onRemoveAttachment={id=>w.removeAttachment(task.id,id)} onDraft={v=>w.setDraft(task.id,v)} onSend={()=>void w.send(task.id)} onAbort={()=>void w.act(()=>host.abort(task.id))} canSend={!task.archived&&!w.sendFailed&&run?.status!=='starting'} running={running} busy={w.busy} disabledReason={task.archived?'任务已归档，请先恢复任务。':undefined}/>
  </>:<><div className="workspace-empty"><FolderOpen size={32}/><h2>{w.project?`你想在 ${w.project.name} 中构建什么？`:'今天，想一起完成什么？'}</h2>{w.project?<div className="starter-actions">{['探索并理解代码','构建新功能','审查代码','修复问题'].map(text=><Button key={text} variant="outline" disabled={creating} onClick={()=>w.setDraft(`project:${w.projectId}`,text)}>{text}</Button>)}</div>:<Button disabled={w.busy} onClick={()=>openName({kind:'project',initial:''})}><Plus/>添加项目</Button>}</div>{firstMessage&&<div className="user-bubble mx-6" role="status">{firstMessage}</div>}{w.project&&<Composer key={`project:${w.projectId}`} taskId={`project:${w.projectId}`} models={w.models} modelValue={w.projectModels[w.projectId]} modelsLoading={w.modelsLoading} modelsError={w.modelsError?.message} onRefreshModels={w.refreshModels} onModel={(provider,id)=>w.setProjectModels(old=>({...old,[w.projectId]:provider+'/'+id}))} roots={w.project.roots} references={w.references[`project:${w.projectId}`]??[]} onReferences={refs=>w.setReferences(old=>({...old,[`project:${w.projectId}`]:refs}))} draft={w.drafts[`project:${w.projectId}`]??''} onDraft={text=>w.setDraft(`project:${w.projectId}`,text)} onSend={()=>void sendFirst()} onAbort={()=>{}} onSettings={()=>setSettings(true)} canSend={!w.project.archived&&!w.modelsLoading&&!w.modelsError&&w.models.some(m=>m.provider+'/'+m.id===w.projectModels[w.projectId])} busy={creating} running={false} disabledReason={w.modelsLoading?'正在加载模型…':w.modelsError?'模型加载失败，请刷新模型列表重试。':!w.models.length?'没有可用模型，请配置模型。':!w.models.some(m=>m.provider+'/'+m.id===w.projectModels[w.projectId])?'所选模型已不可用，请重新选择模型。':undefined}/>}</>}
 </main>;
 return <TooltipProvider><div className="workbench">{narrow?content:<AppFrame sidebar={side} collapsed={!sidebar} onExpand={()=>setSidebar(true)} onNew={newConversation} onSearch={()=>{setSidebar(true);requestAnimationFrame(()=>document.querySelector<HTMLInputElement>('[aria-label="搜索任务"]')?.focus());}} onSettings={()=>{remember();setSettings(true);}} inspector={inspector} inspectorOpen={details} onCloseInspector={()=>setDetails(false)}>{content}</AppFrame>}</div>
  ...
  <RuntimeSettings onSaved={async()=>{await w.refreshModels();await w.refresh();}} open={settings} onClose={()=>setSettings(false)} theme={theme.theme} onTheme={theme.change} themeError={theme.error} restoreFocus={restore}/>
