...
export function App(){
 ...
 const content=<main ref={main} tabIndex={-1} className="conversation">
  ...
  {task?<><MessageList taskKey={key} messages={w.messages} streamingText={running||w.historyBusy?run?.text??'':''} running={running} hasMore={!!w.cursor} loading={w.historyBusy} onMore={()=>{if(ready)void w.history(true);}} onRefresh={()=>{if(ready)void w.history();}} onError={e=>w.setError(hostError(e))}/>
   ...
   <Composer key={`composer:${task.id}`} taskId={task.id} roots={task.roots} projectName={w.project?.name} references={w.references[task.id]??[]} onReferences={refs=>w.setReferences(old=>({...old,[task.id]:refs}))} models={capabilities?.models} onModel={(p,id)=>void w.act(()=>records.model(task.id,p,id))} draft={w.drafts[task.id]??''} onFiles={()=>setDetails(true)} onContinue={()=>void w.act(()=>records.resume(task.id))} onSettings={()=>setSettings(true)} modelLabel={capabilities?.state?.model?.id??task.model?.split('/').at(-1)} attachmentIds={w.attachments[task.id]??[]} onRemoveAttachment={id=>w.removeAttachment(task.id,id)} onDraft={v=>w.setDraft(task.id,v)} onSend={()=>void w.send(task.id)} onAbort={()=>void w.act(()=>host.abort(task.id))} canSend={!task.archived&&!w.sendFailed&&run?.status!=='starting'} running={running} busy={w.busy} disabledReason={task.archived?'任务已归档，请先恢复任务。':undefined}/>
  </>:<><div className="workspace-empty"><FolderOpen size={32}/><h2>{w.project?`你想在 ${w.project.name} 中构建什么？`:'今天，想一起完成什么？'}</h2>{w.project?<div className="starter-actions">{['探索并理解代码','构建新功能','审查代码','修复问题'].map(text=><Button key={text} variant="outline" disabled={creating} onClick={()=>w.setDraft(`project:${w.projectId}`,text)}>{text}</Button>)}</div>:<Button disabled={w.busy} onClick={()=>openName({kind:'project',initial:''})}><Plus/>添加项目</Button>}</div>{firstMessage&&<div className="user-bubble mx-6" role="status">{firstMessage}</div>}{w.project&&<Composer key={`project:${w.projectId}`} taskId={`project:${w.projectId}`} projectName={w.project.name} roots={w.project.roots} references={w.references[`project:${w.projectId}`]??[]} onReferences={refs=>w.setReferences(old=>({...old,[`project:${w.projectId}`]:refs}))} draft={w.drafts[`project:${w.projectId}`]??''} onDraft={text=>w.setDraft(`project:${w.projectId}`,text)} onSend={()=>void sendFirst()} onAbort={()=>{}} onSettings={()=>setSettings(true)} canSend={!w.project.archived} busy={creating} running={false}/>}</>}
 </main>;
