...
import type {ApprovalMode,Message,TaskRecord,HostError} from '../../../packages/host-contract/src';
...
import {usePreferences} from '@/components/workbench/preferences';
...
import {SessionCatalog} from '@/components/workbench/session-catalog';
...
export function App(){
 const w=useWorkbench(),theme=useTheme(),preference=usePreferences();
 ...
 const [settings,setSettings]=useState(false),[catalogOpen,setCatalogOpen]=useState(false),[recovery,setRecovery]=useState<'task'|'project'|null>(null),[trajectoryOpen,setTrajectoryOpen]=useState(false);
 ...
 const [draftThinking,setDraftThinking]=useState<Record<string,string|null>>({});
 const [thinkingNotice,setThinkingNotice]=useState('');
 useEffect(()=>setThinkingNotice(''),[w.taskId,w.projectId]);
 const [creating,setCreating]=useState(false);
 const [firstTurn,setFirstTurn]=useState<{projectId:string;taskId?:string;text:string;timestamp:number}|null>(null);
 const newConversation=()=>{if(w.project){w.chooseProject(w.project.id);}else openName({kind:'project',initial:''});};
 const sendFirst=async()=>{const projectId=w.projectId;const draftKey=`project:${projectId}`;const text=w.drafts[draftKey]??'';if(!text.trim()||creating)return;const refs=w.references[draftKey]??[];const turn={projectId,text,timestamp:Date.now()};setCreating(true);setFirstTurn(turn);w.setDraft(draftKey,'');w.setReferences(old=>({...old,[draftKey]:[]}));const selected=w.projectModels[projectId];if(!selected||!w.models.some(m=>m.provider+'/'+m.id===selected)){setCreating(false);setFirstTurn(null);w.setDraft(draftKey,text);w.setReferences(old=>({...old,[draftKey]:refs}));return;}try{const created=await records.create(projectId,text.trim().slice(0,80),'shared',selected,true);setFirstTurn({...turn,taskId:created.id});if(approvalPreference.mode)await records.approval(created.id,approvalPreference.mode);if(draftThinking[projectId])await records.setThinking(created.id,draftThinking[projectId]);await w.refresh();w.chooseTask(created);await w.send(created.id,false,text,refs);}catch(e){w.setError(hostError(e));w.setDraft(draftKey,text);w.setReferences(old=>({...old,[draftKey]:refs}));}finally{setCreating(false);setFirstTurn(null);}};
 ...
 useEffect(()=>{if(!task||!run)return;const editor=run.extensionUi?.editorText;if(editor!=null)w.setDraft(task.id,editor);document.title=run.extensionUi?.title?`${run.extensionUi.title} · Cool PI`:'Cool PI';const last=notified.current[run.runId]??0;const events=run.events.filter(event=>event.seq>last&&event.eventType==='extension_ui_request'&&(event.payload as any)?.method==='notify');for(const event of events){const payload=event.payload as any;void desktopPlatformShell.notify(payload.notifyType==='error'?'OMP 错误':payload.notifyType==='warning'?'OMP 警告':'OMP 通知',{body:payload.message});}notified.current[run.runId]=run.seq;},[task?.id,run?.runId,run?.seq,run?.extensionUi?.editorText,run?.extensionUi?.title]);
 const completionStates=useRef<Record<string,string>>({});
 useEffect(()=>{for(const candidate of w.runs){const previous=completionStates.current[candidate.runId];completionStates.current[candidate.runId]=candidate.status;if(previous&&['running','compacting','retrying','pending_ui'].includes(previous)&&candidate.status==='idle'){const finished=w.tasks.find(item=>item.id===candidate.taskId);if(preference.preferences.sound)void desktopPlatformShell.playCompletionSound();if(preference.preferences.notifications)void desktopPlatformShell.notify('OMP 已完成',{body:finished?.title??'会话已完成'});}}},[w.runs,w.tasks,preference.preferences.sound,preference.preferences.notifications]);
 ...
 const ready=!external&&!!run&&['ready','idle','interrupted'].includes(run.status),running=!external&&!!run&&['running','compacting','retrying','pending_ui'].includes(run.status);
 ...
 const side=<ProjectSidebar onCopySessionId={t=>void copySessionId(t)} runtimeTasks={runtime.tasks} onRuntime={openRuntime} onRuntimeManagement={()=>openSettings('runtime')} projects={w.projects} tasks={w.tasks} runs={w.runs} projectId={w.projectId} taskId={w.taskId} loading={w.loading} busy={w.busy} showArchived={archived} onArchived={setArchived}
  ...
  onSettings={()=>{openSettings();}} onCatalog={()=>{remember();setCatalogOpen(true);setDrawer(false);}} onClose={()=>narrow?setDrawer(false):setSidebar(false)} onPaths={()=>{remember();setRecovery('project');}}/>;
 ...
 const content=<main ref={main} tabIndex={-1} className="conversation">
  ...
  {task?<>{trajectoryOpen?
   ...
   <Composer availableCommands={run?.availableCommands} queuedCount={run?.authoritativeQueuedCount} onQueue={action=>void w.queueMessage(task.id,action)} thinking={task.thinking} onThinking={level=>void w.act(()=>records.setThinking(task.id,level))} thinkingDisabled={!!run&&['starting','ready','idle','running','interrupted'].includes(run.status)||task.archived} modelDisabled={external} key={`composer:${task.id}`} taskId={task.id} approvalMode={task.approvalMode} onApprovalMode={mode=>changeApproval(task.id,mode)} approvalSwitching={approvalSwitching===task.id} approvalDisabled={external||task.archived||run?.status==='starting'||!!run?.pendingUi?.length||!!w.sendFailed} roots={task.roots} references={w.references[task.id]??[]} onReferences={refs=>w.setReferences(old=>({...old,[task.id]:refs}))} models={ready||running?capabilities?.models??[]:w.models} modelValue={ready||running?capabilities?.state?.model?capabilities.state.model.provider+'/'+capabilities.state.model.id:undefined:task.model??undefined} modelsLoading={!ready&&!running&&w.modelsLoading} modelsError={!ready&&!running?w.modelsError?.message:undefined} onRefreshModels={ready||running?undefined:w.refreshModels} onModel={(p,id)=>void w.act(async()=>{await records.model(task.id,p,id);const updated=await records.thinking(task.id);setThinkingNotice(task.thinking&&!updated?'模型已切换；不兼容的推理强度已清除，使用 OMP 默认。':'');})} draft={w.drafts[task.id]??''} onFiles={()=>setDetails(true)} onContinue={external?undefined:()=>void w.act(()=>records.resume(task.id))} onSettings={()=>openSettings()} modelLabel={ready||running?capabilities?.state?.model?.id:undefined} attachmentIds={w.attachments[task.id]??[]} onRemoveAttachment={id=>w.removeAttachment(task.id,id)} onDraft={v=>w.setDraft(task.id,v)} onSend={()=>{if(!external)void w.send(task.id);}} onAbort={()=>void w.act(()=>host.abort(task.id))} canSend={!external&&!task.archived&&!w.sendFailed} running={running} busy={w.busy} disabledReason={external?'终端正在接管此会话，退出终端后可继续发送。':task.archived?'任务已归档，请先恢复任务。':undefined}/>
  </>:<>{firstTurn?.projectId===w.projectId?<MessageList taskKey={`new:${firstTurn.projectId}`} messages={[{role:'user',content:firstTurn.text,timestamp:firstTurn.timestamp}]} streamingText="" running={false} hasMore={false} loading={false} onMore={()=>{}} onError={e=>w.setError(hostError(e))}/>:<div className="workspace-empty"><FolderOpen size={32}/><h2>{w.project?`你想在 ${w.project.name} 中构建什么？`:'今天，想一起完成什么？'}</h2>{w.project?<div className="starter-actions">{['探索并理解代码','构建新功能','审查代码','修复问题'].map(text=><Button key={text} variant="outline" disabled={creating} onClick={()=>w.setDraft(`project:${w.projectId}`,text)}>{text}</Button>)}</div>:<Button disabled={w.busy} onClick={()=>openName({kind:'project',initial:''})}><Plus/>添加项目</Button>}</div>}{w.project&&<Composer thinking={draftThinking[w.projectId]} onThinking={level=>setDraftThinking(old=>({...old,[w.projectId]:level}))} key={`project:${w.projectId}`} taskId={`project:${w.projectId}`} approvalMode={approvalPreference.mode} onApprovalMode={saveApprovalPreference} models={w.models} modelValue={w.projectModels[w.projectId]} modelsLoading={w.modelsLoading} modelsError={w.modelsError?.message} onRefreshModels={w.refreshModels} onModel={(provider,id)=>void w.act(async()=>{const projectId=w.projectId;await w.selectProjectModel(provider,id);const model=w.models.find(m=>m.provider===provider&&m.id===id);const level=draftThinking[projectId];if(level&&!model?.capability?.thinking.levels.includes(level as import('../../../packages/host-contract/src').ThinkingLevel)){setDraftThinking(old=>({...old,[projectId]:null}));setThinkingNotice('模型已切换；不兼容的推理强度已清除。');}})} roots={w.project.roots} references={w.references[`project:${w.projectId}`]??[]} onReferences={refs=>w.setReferences(old=>({...old,[`project:${w.projectId}`]:refs}))} draft={w.drafts[`project:${w.projectId}`]??''} onDraft={text=>w.setDraft(`project:${w.projectId}`,text)} onSend={()=>void sendFirst()} onAbort={()=>{}} onSettings={()=>openSettings()} canSend={!w.project.archived&&!w.modelsLoading&&!w.modelsError&&w.models.some(m=>m.provider+'/'+m.id===w.projectModels[w.projectId])} busy={creating||w.busy} running={false} disabledReason={w.modelsLoading?'正在加载模型…':w.modelsError?'模型加载失败，请刷新模型列表重试。':!w.models.length?'没有可用模型，请配置模型。':!w.models.some(m=>m.provider+'/'+m.id===w.projectModels[w.projectId])?'请选择模型后开始对话。':undefined}/>}</>}
 </main>;
 return <TooltipProvider><div className="workbench">{narrow?content:<AppFrame sidebar={side} collapsed={!sidebar} onExpand={()=>setSidebar(true)} onNew={newConversation} onSearch={()=>{setSidebar(true);requestAnimationFrame(()=>document.querySelector<HTMLInputElement>('[aria-label="搜索任务"]')?.focus());}} onSettings={()=>{openSettings();}} inspector={inspector} inspectorOpen={details} onCloseInspector={()=>setDetails(false)}>{content}</AppFrame>}</div>
  ...
  {(dialog?.kind==='project'||dialog?.project)?<LocalProjectDialog project={dialog?.project?w.project:undefined} onClose={()=>setDialog(null)} onCreated={async p=>{await w.refresh();if(!dialog?.project)w.chooseProject(p.id);}}/>:dialog&&<NameDialog {...dialog} onGenerate={dialog.task?()=>records.suggestTitle(dialog.task!.id):undefined} onClose={()=>setDialog(null)} restoreFocus={restore} onSubmit={async(name,trusted,mode)=>{if(dialog.kind==='task'){const t=await records.create(w.projectId,name,mode);await w.refresh();w.chooseTask(t);setDrawer(false);}else if(dialog.task){await records.update({...dialog.task,title:name});await w.refresh();}else if(dialog.project&&w.project){await projects.update(w.project.id,name,w.project.archived);await w.refresh();}}}/>}
  <RuntimeSettings tab={settingsTab} onTab={setSettingsTab} runtimeTasks={runtime.tasks} tasks={w.tasks} projects={w.projects} projectId={w.projectId} taskId={task?.id} runtimeError={runtime.error} onRefreshRuntime={runtime.refresh} onOpenTask={target=>{setSettings(false);openRuntime(target);}} onProviderLogin={()=>{setSettings(false);if(task)openRuntime(task);}} onSaved={async()=>{await w.refreshModels();await w.refresh();}} open={settings} onClose={()=>setSettings(false)} theme={theme.theme} onTheme={theme.change} themeError={theme.error} preferences={preference.preferences} onPreferences={preference.change} preferencesError={preference.error} restoreFocus={restore}/>
  <SessionCatalog open={catalogOpen} onOpenChange={value=>{setCatalogOpen(value);if(!value)restore();}} onOpenTask={taskId=>{const target=w.tasks.find(item=>item.id===taskId);if(target)w.chooseTask(target);}}/>
