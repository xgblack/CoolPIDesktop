...
export function App(){
 ...
 const content=<main ref={main} tabIndex={-1} className="conversation">
  ...
  {task?<><div className="flex items-center justify-end border-b px-3 py-1"><Button variant="ghost" size="sm" onClick={()=>setTrajectoryOpen(v=>!v)} aria-pressed={trajectoryOpen}>{trajectoryOpen?'对话':'轨迹'}</Button></div>{trajectoryOpen?<TrajectoryView taskId={task.id} run={run} onError={e=>w.setError(hostError(e))}/>:<MessageList tools={run?.tools} taskKey={key} messages={w.messages} streamingText={running||w.historyBusy?run?.text??'':''} running={running} hasMore={!!w.cursor} loading={w.historyBusy} onMore={()=>{if(!running)void w.history(true);}} onRefresh={()=>{if(!running)void w.history();}} onError={e=>w.setError(hostError(e))}/>}
