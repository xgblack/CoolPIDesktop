...
function UiRequest({request,complete}:{request:PendingUiRequest;complete:(value:string|null,confirmed:boolean|null,cancelled:boolean)=>void}) {
  ...
  return <fieldset className="ui-request"><legend>{request.title||request.method}</legend><p>{request.message}</p>
    ...
    {request.method!=='confirm'&&<button disabled={!value} onClick={()=>complete(value,null,false)}>提交</button>}
...
export function App(){
  ...
  useEffect(()=>{let active=true;const refresh=()=>host.list().then(list=>{if(active)setTasks(current=>list.reduce((next,snapshot)=>applySnapshot(next,snapshot),current));}).catch(reason=>{if(active)setError(hostError(reason).message);});refresh();const timer=setInterval(refresh,700);return()=>{active=false;clearInterval(timer);};},[]);
  const task=tasks[selected];
  const act=async(action:()=>Promise<unknown>)=>{setBusy(true);setError('');try{const result=await action();if(result&&typeof result==='object'&&'taskId' in result){const snapshot=result as TaskSnapshot;setTasks(current=>applySnapshot(current,snapshot));setSelected(snapshot.taskId);}}catch(reason){setError(hostError(reason).message);}finally{setBusy(false);}};
  ...
  return <main><header><h1>OMP Desktop</h1><span>本机 Runtime</span></header>
    ...
    <div className="workspace"><aside><h2>任务 <button title="新任务" disabled={busy} onClick={()=>act(()=>host.start(crypto.randomUUID(),executable))}>+</button></h2>{Object.values(tasks).map(item=><button className={item.taskId===selected?'selected':''} key={item.taskId} onClick={()=>setSelected(item.taskId)}>{item.taskId.slice(0,8)} · {item.status}</button>)}</aside>
      <section>{task?<><div className="task-heading"><h2>{task.status}</h2><button disabled={busy||task.status!=='running'} onClick={()=>act(()=>host.abort(task.taskId))}>取消</button><button disabled={busy} onClick={()=>act(()=>host.stop(task.taskId))}>停止</button><button disabled={busy} onClick={()=>act(()=>host.restart(task.taskId))}>重启</button></div>
        ...
        <pre>{task.events.map(eventText).filter(Boolean).join('')||task.text||'等待输出'}</pre><form onSubmit={event=>{event.preventDefault();if(message.trim())act(async()=>{const result=await host.prompt(task.taskId,message);setMessage('');return result;});}}><textarea value={message} placeholder="输入消息" onChange={event=>setMessage(event.target.value)}/><button disabled={busy||!message.trim()||task.status==='running'}>发送</button></form>
