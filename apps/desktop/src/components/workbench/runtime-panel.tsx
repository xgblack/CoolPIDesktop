...
export function RuntimePanel({task,run,info,error,onRefresh,current=true}:RuntimePanelProps){
 const [busy,setBusy]=useState(false),[localError,setError]=useState<HostError|null>(null),[confirmation,setConfirmation]=useState<Action|null>(null),[command,setCommand]=useState(''),[copied,setCopied]=useState(false);
