...
export function ledgerRows(records:TrajectoryRecord[],collapsed:Set<number>,foldTools:boolean,matches:Set<string>|null,range:Set<string>|null):LedgerRow[] {
 ...
 for(const r of records){
  ...
  if(r.turn!==turn){turn=r.turn;step=undefined;rows.push({key:`turn:${r.turn}`,type:'turn',record:r,label:r.turn==null?'会话上下文':`轮次 ${r.turn}`,count:counts.get(r.turn),depth:0});}
  if(!matches&&r.turn!=null&&collapsed.has(r.turn))continue;
  if(r.step!==step&&r.step!=null){step=r.step;rows.push({key:`step:${r.turn}:${r.step}`,type:'step',record:r,label:`生成步骤 ${r.step}`,depth:0});}
  let depth=0;let parent=r.parentId;const visited=new Set([r.id]);let toolAncestor=false;
  while(parent&&ids.has(parent)&&!visited.has(parent)){visited.add(parent);const p=ids.get(parent)!;toolAncestor ||= p.kind==='tool';depth++;parent=p.parentId;}
  if(foldTools&&toolAncestor&&!matches)continue;
...
export function timelineItems(records:TrajectoryRecord[],duration:boolean):TimelineItem[]{
 let offset=0;
 return records.map(r=>{const width=duration?(r.status==='running'?0:Math.max(0,r.durationMs??0)):1;const item={record:r,start:offset,end:offset+width,lane:r.kind==='assistant'?1:r.kind==='tool'?2:0};offset+=width;return item;});
}
