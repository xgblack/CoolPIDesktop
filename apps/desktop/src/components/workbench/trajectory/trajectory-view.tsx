...
function TrajectorySession({taskId,run,onError}:TrajectoryViewProps){
 ...
 {data.loading&&!data.records.length&&!record?<div className="trajectory-state">正在定位最新轨迹…</div>:!data.records.length&&!record?<div className="trajectory-state">暂无轨迹记录</div>:<div className="trajectory-body">
 ...
 <button className="content" role="cell" aria-label={`检查 ${r.name} ${r.id}`} onClick={()=>{setFollow(false);setSelected(r.id);}}><span className="trajectory-record-name">{r.kind==='tool'||r.kind==='system'?r.name+' · ':''}</span>{previewText(r.content)||previewText(r.output)||(r.images.length?`${r.images.length} 张图片`:statusLabels[r.status])}<span className="trajectory-status" data-status={r.status}>{statusLabels[r.status]}</span></button>
