...
function TrajectorySession({taskId,run,onError}:TrajectoryViewProps){
 ...
 const follow=useRef(true),previousSize=useRef(0),cache=useRef<ImageCache>(new Map()),scroll=useRef<HTMLDivElement>(null),anchor=useRef<{key:string;offset:number}|null>(null),focusPending=useRef<string|null>(null);
 ...
 useLayoutEffect(()=>{
  ...
  const oldSize=previousSize.current;const nextSize=virtual.getTotalSize();
  if(oldSize>0&&!follow.current&&!anchor.current&&!focusPending.current&&nextSize!==oldSize){scroll.current.scrollTop+=nextSize-oldSize;}
 ...
 const details=record?<TrajectoryDetails taskId={taskId} record={record} cache={cache.current} onError={onError} onClose={()=>setSelected(null)}/>:null;
 ...
 <TrajectoryTimeline records={data.records} duration={duration} onFocus={select} onRange={ids=>{if(ids)setFollow(false);setRange(ids);}}/>
 ...
 <div ref={scroll} className="trajectory-ledger" role="table" aria-label="轨迹记录表" aria-rowcount={rows.length} onWheel={e=>{if(e.deltaY<0)setFollow(false);}} onKeyDown={e=>{if(['ArrowUp','PageUp','Home'].includes(e.key))setFollow(false);if(e.key==='End')tail();}} onScroll={e=>{const el=e.currentTarget;const atEnd=el.scrollHeight-el.scrollTop-el.clientHeight<12;if(atEnd&&!matches&&!range)setFollow(true);else if(el.scrollTop>0)setFollow(false);}} tabIndex={0}>
 {!rows.length&&<div className="trajectory-state">没有匹配记录</div>}
 <div style={{height:virtual.getTotalSize(),position:'relative'}}>{virtual.getVirtualItems().map(item=>{const row=rows[item.index],r=row.record;return <div key={row.key} data-index={item.index} style={{position:'absolute',top:0,left:0,width:'100%',height:item.size,transform:`translateY(${item.start}px)`}} role="row" aria-rowindex={item.index+1}>
 {row.type==='turn'?<button className="trajectory-turn" aria-expanded={r.turn==null||!collapsed.has(r.turn)} onClick={()=>{if(r.turn!=null)setCollapsed(old=>{const s=new Set(old);s.has(r.turn!)?s.delete(r.turn!):s.add(r.turn!);return s;});}}>{r.turn!=null&&collapsed.has(r.turn)?<ChevronRight size={12}/>:<ChevronDown size={12}/>}<span>{row.label}</span><span>{row.count} 条</span></button>:row.type==='step'?<div className="trajectory-step">{row.label}</div>:<div className={`trajectory-row ${record?.id===r.id?'selected':''}`} data-kind={r.kind} data-record-id={r.id}>
 ...
 {narrow&&<Sheet open={!!record} onOpenChange={open=>{if(!open)setSelected(null);}}><SheetContent className="trajectory-sheet"><SheetTitle className="sr-only">轨迹记录详情</SheetTitle><SheetDescription className="sr-only">选中记录的内容、用量、计时与附件</SheetDescription>{details}</SheetContent></Sheet>}
 </section>;
}
