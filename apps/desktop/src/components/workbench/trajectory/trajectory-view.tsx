...
function TrajectorySession({taskId,run,onError}:TrajectoryViewProps){
 ...
 <TrajectoryTimeline records={data.records} duration={duration} selectedId={selected} range={range} onFocus={select} onRange={ids=>{if(ids){setFollow(false);setSelected(null);}setRange(ids);}}/>
