...
export function TrajectoryDetails({taskId,record,capabilities,cache,onError,onClose}:{taskId:string;record:TrajectoryRecord;capabilities?:unknown;cache:ImageCache;onError:(e:unknown)=>void;onClose:()=>void}){
 ...
 <Tabs.Root value={tab} onValueChange={setTab}><Tabs.List className="trajectory-detail-tabs" aria-label="记录详情分类">{[['overview','概览'],...(record.input!=null?[['input','参数']]:[]),...(record.output!=null?[['output','结果']]:[]),['schema','Schema'],['timing','计时']].map(([id,label])=><Tabs.Trigger key={id} value={id}>{label}</Tabs.Trigger>)}</Tabs.List>
 <Tabs.Content value="overview">{metadata(entries.filter(([k])=>!['开始','结束','耗时','TTFT','计时来源'].includes(k)))}
 ...
 <details><summary>原始投影记录</summary><button onClick={()=>void navigator.clipboard.writeText(stringify(record)).catch(onError)}>复制记录</button><pre>{stringify(record)}</pre></details></Tabs.Content>
