...
export function RuntimeSettings({open,onClose,theme,onTheme,themeError,restoreFocus,onSaved}:{open:boolean;onClose:()=>void;theme:Theme;onTheme:(t:Theme)=>void;themeError:string;restoreFocus:()=>void;onSaved:()=>Promise<unknown>}){
 ...
 return <Dialog open={open} onOpenChange={v=>{if(!v){if(modelDirty)setConfirmClose(true);else onClose();}}}><DialogContent className="sm:max-w-3xl" onCloseAutoFocus={e=>{e.preventDefault();restoreFocus();}}><DialogHeader><DialogTitle>设置</DialogTitle><DialogDescription>管理工作台外观和系统安装的 OMP。</DialogDescription></DialogHeader>
  ...
  <div className="flex gap-2"><Button variant={tab==='models'?'secondary':'ghost'} onClick={()=>setTab('models')}>模型配置</Button><Button variant={tab==='general'?'secondary':'ghost'} onClick={()=>setTab('general')}>外观与运行时</Button></div>
  <div hidden={tab!=='models'}>{open&&<ModelSettings onSaved={onSaved} onDirtyChange={setModelDirty}/>}</div><div hidden={tab!=='general'}>
