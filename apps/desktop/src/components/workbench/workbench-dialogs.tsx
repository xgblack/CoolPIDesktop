...
export function RuntimeSettings({open,onClose,theme,onTheme,themeError,restoreFocus}:{open:boolean;onClose:()=>void;theme:Theme;onTheme:(t:Theme)=>void;themeError:string;restoreFocus:()=>void}){
 ...
 return <Dialog open={open} onOpenChange={v=>{if(!v)onClose();}}><DialogContent className="sm:max-w-xl" onCloseAutoFocus={e=>{e.preventDefault();restoreFocus();}}><DialogHeader><DialogTitle>设置</DialogTitle><DialogDescription>管理工作台外观和系统安装的 OMP。</DialogDescription></DialogHeader>
  ...
  <p className="text-xs text-muted-foreground">模型暂通过 OMP 配置；工作台仅选择已有模型。</p>
