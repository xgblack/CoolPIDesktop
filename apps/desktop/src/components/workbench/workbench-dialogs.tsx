...
import type {HostError,RuntimeInfo,RuntimeTaskInfo,TaskRecord} from '../../../../../packages/host-contract/src';
...
export function RuntimeSettings({tab,onTab:setTab,runtimeTasks,tasks,runtimeError,onRefreshRuntime,onOpenTask,open,onClose,theme,onTheme,themeError,restoreFocus,onSaved}: {tab:SettingsTab;onTab:(tab:SettingsTab)=>void;runtimeTasks:RuntimeTaskInfo[];tasks:TaskRecord[];runtimeError:HostError|null;onRefreshRuntime:()=>Promise<void>;onOpenTask:(task:TaskRecord)=>void;open:boolean;onClose:()=>void;theme:Theme;onTheme:(t:Theme)=>void;themeError:string;restoreFocus:()=>void;onSaved:()=>Promise<unknown>}){
 ...
 return <Dialog open={open} onOpenChange={v=>{if(!v){if(settingsDirty)setConfirmClose(true);else onClose();}}}><DialogContent className="settings-dialog" onCloseAutoFocus={e=>{e.preventDefault();restoreFocus();}}><DialogHeader><DialogTitle>设置</DialogTitle><DialogDescription>配置工作台偏好与 OMP 运行时。</DialogDescription></DialogHeader>
  ...
  <div className="settings-body" hidden={tab!=='models'}><h2>模型配置</h2><p className="settings-intro">连接你的模型，开始工作。</p>{open&&<ModelSettings onSaved={onSaved} onDirtyChange={setModelDirty}/>}</div><div className="settings-body" hidden={tab!=='general'}><h2>外观</h2><p className="settings-intro">调整工作台的显示主题。</p>
  <section className="settings-section settings-section-plain" aria-label="外观主题"><div className="theme-options">{([['light','浅色',Sun],['dark','深色',Moon],['system','跟随系统',Monitor]] as const).map(([value,label,Icon])=><Button key={value} variant={theme===value?'secondary':'outline'} aria-pressed={theme===value} onClick={()=>onTheme(value)}><Icon/>{label}</Button>)}</div>{themeError&&<p role="alert">{themeError}</p>}</section>
  </div>
