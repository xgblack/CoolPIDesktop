...
export function ProjectSidebar({projects,tasks,runs,projectId,taskId,loading,busy,showArchived,onArchived,onProject,onTask,onNewProject,onNewTask,onRenameProject,onArchiveProject,onRenameTask,onUpdateTask,onSettings,onClose,onPaths}: {
 ...
 return <aside className="project-sidebar" aria-label="项目与任务">
  ...
  <div className="sidebar-primary"><Button variant="ghost" onClick={onNewTask} disabled={!project||project.archived||busy}><Plus/>新建任务<span>⌘ N</span></Button><input aria-label="搜索任务" placeholder="搜索任务…" value={query} onChange={e=>setQuery(e.target.value)}/></div>
  ...
  <nav className="task-list" aria-label="任务列表">
   {loading?<div aria-label="正在加载项目" className="space-y-3 p-3">{[0,1,2,3].map(i=><div key={i} className="h-8 rounded bg-muted animate-pulse"/>)}</div>:!project?<div className="sidebar-empty"><FolderOpen/><p>把工作目录添加为项目，开始第一段对话。</p><Button size="sm" variant="outline" onClick={onNewProject}>添加项目</Button></div>:visible.length===0?<div className="sidebar-empty"><MessageSquare/><p>{query?'没有匹配的任务':'这个项目还没有任务'}</p><Button size="sm" variant="outline" disabled={project.archived} onClick={onNewTask}>创建任务</Button></div>:visible.sort((a,b)=>Number(b.pinned)-Number(a.pinned)).map(t=>{
