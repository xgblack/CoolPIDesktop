...
export function useWorkbench(){
 ...
 const refreshModels=useCallback(()=>{
  if(modelRequest.current)return modelRequest.current;
  setModelsLoading(true);setModelsError(null);
  const request=modelConfig.verify().then(result=>{if(active.current)setModels(result.models);}).catch(e=>{if(active.current)setModelsError(hostError(e));}).finally(()=>{modelRequest.current=null;if(active.current)setModelsLoading(false);});
  modelRequest.current=request;return request;
 },[]);
 ...
 useEffect(()=>{
  ...
  const flush=()=>{frame=0;const events=queue;queue=[];setRuns(old=>{let next=old;for(const event of events){const s=next.find(s=>s.taskId===event.taskId);if(!s||s.runId!==event.runId||event.seq>s.seq+1){recover(event.taskId);continue;}next=next.map(s=>s.taskId===event.taskId?applyEvent(s,event):s);if(!['message_update','stderr'].includes(event.eventType))recover(event.taskId);}return next;});};
