...
1. 打开“设置 → 外观与运行时”，点击“检测 OMP”。路径留空时使用已保存路径或 PATH；显式指定路径优先。
...
默认编辑 `~/.omp/agent/models.yml`；不存在时使用同目录的 `models.yaml`。仅存在旧 `models.json` 时，需先由 OMP 完成官方迁移。模型目录搜索命中、配置保存、OMP 加载和真实连接是不同结果，应分别检查。
